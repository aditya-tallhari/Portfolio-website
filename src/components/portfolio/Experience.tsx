...
export const Experience = () => {
  ...
  return (
    <section 
      ref={containerRef} 
      id="experience" 
      className="relative pt-40 pb-16 md:pt-48 md:pb-20 px-6 md:px-12 overflow-hidden"
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        {/* ═══════════════════════════════════════════════
           ...
           ═══════════════════════════════════════════════ */}
        <header ref={headerRef} className="mb-24 md:mb-40 relative">
          {/* Section ID BG */}
        ...
        <div ref={timelineTrackRef} className="relative">
          ...
          <div 
            ref={timelineFillRef} 
            className="absolute top-0 left-8 md:left-1/2 w-0.5 -translate-x-1/2 bg-[var(--accent-primary)] origin-top opacity-60 filter blur-[0.5px]" 
              ...
              experiencesList.map((exp, i) => {
                ...
                return (
                  <div key={i} className="exp-row relative">
                    ...
                    <div className={`flex flex-col md:flex-row items-start md:items-center gap-12 md:gap-0 ${isEven ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                      
                      {/* Magnetic Card Container */}
                      <div 
                        className={`exp-card relative w-[calc(100%-3.5rem)] ml-14 md:w-[46%] md:ml-0 group cursor-default`}
                        data-index={i}
                      ...
                      >
                        ...
                        <div className="exp-card-inner relative p-6 sm:p-8 md:p-12 rounded-[1.5rem] sm:rounded-[2rem] bg-[var(--text-primary)]/[0.04] border border-[var(--border-primary)] backdrop-blur-3xl transition-all duration-700 hover:border-[var(--accent-primary)]/40 hover:shadow-[0_30px_100px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_30px_100px_rgba(0,0,0,0.5)] overflow-hidden">
                          ...
                          <div className="exp-card-content relative z-10 flex flex-col gap-8">
                            ...
                            <div className="space-y-3">
                              <h3 className="text-2xl sm:text-3xl md:text-4xl font-playfair font-black tracking-tighter uppercase leading-[0.9] text-[var(--text-primary)] group-hover:text-[var(--accent-primary)] transition-colors duration-500">
                                 {exp.role}
                              </h3>
                              <div className="flex items-center gap-2 sm:gap-4 flex-wrap">
                                 <span className="text-[11px] sm:text-[12px] font-bold uppercase tracking-wider text-[var(--accent-primary)] opacity-80">
                                    {exp.company}
                                 </span>
                                 <div className="w-1 h-1 rounded-full bg-[var(--border-primary)]" />
                                 <span className="flex items-center gap-2 text-[9px] sm:text-[10px] font-medium uppercase tracking-widest text-[var(--text-primary)] opacity-40">
                            ...
                            <ul className="flex flex-col gap-3 sm:gap-4 font-jetbrains">
                               {exp.points.map((p, j) => (
                                 <li key={j} className="exp-bullet flex items-start gap-3 sm:gap-4 text-[12px] sm:text-[13px] leading-relaxed text-[var(--text-secondary)]/80 hover:text-[var(--text-primary)] transition-colors group/bullet">
                                    <ChevronRight size={14} className="mt-0.5 sm:mt-1 shrink-0 text-[var(--accent-primary)] opacity-30 group-hover/bullet:opacity-100 group-hover/bullet:translate-x-1 transition-all" />
                                    <span>{p}</span>
                                 </li>
                               ))}
                            </ul>
                          ...
                          {/* Floating Aesthetic Mark Removed */}
                        </div>
                      </div>
                      ...
                      <div className={`exp-ghost-num absolute md:relative top-0 right-0 md:top-auto md:right-auto flex md:w-[46%] items-center z-[-1] md:z-auto ${isEven ? 'justify-end md:justify-start md:pl-12' : 'justify-end md:pr-12'}`}>
                         <span className="text-[40vw] md:text-[12vw] font-playfair font-black text-[var(--text-primary)] select-none pointer-events-none tracking-tighter leading-none opacity-[0.03] md:opacity-[0.08] transition-opacity duration-500">
                           {exp.id}
                         </span>
                      </div>
