...
const platforms = [
  {
    ...
    streak: '50',
    color: '#FFA116',
    link: 'https://leetcode.com/u/aditya-tallhari/',
    icon: (
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="mb-20">
          ...
          <motion.p
            ...
            className="max-w-2xl text-sm md:text-base text-[var(--text-primary)] opacity-60 leading-relaxed font-jetbrains"
          >
        ...
        <motion.div
          ...
          className="mt-20 flex flex-wrap items-center justify-between gap-8 pt-8 border-t border-[var(--border-primary)] opacity-40"
        >
          <div className="flex items-center gap-2 font-jetbrains text-[9px] uppercase tracking-widest font-bold text-[var(--text-primary)]">
      ...
      <div className="flex-1 p-10 rounded-[2rem] border border-[var(--border-primary)] bg-[var(--text-primary)]/[0.02] backdrop-blur-sm flex flex-col justify-between group-hover:border-[var(--accent-primary)]/20 transition-all duration-500 overflow-hidden relative">
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-[var(--text-primary)]/[0.04] border border-[var(--border-primary)] w-fit">
                  ...
                  <span className="text-[9px] font-bold text-[var(--text-primary)] opacity-60 uppercase tracking-widest font-jetbrains">{platform.id}</span>
                </div>
              </div>
            </div>
            <a
              ...
              className="w-10 h-10 rounded-full border border-[var(--border-primary)] flex items-center justify-center text-[var(--text-primary)] opacity-40 group-hover:opacity-100 group-hover:text-[var(--accent-primary)] group-hover:border-[var(--accent-primary)]/40 transition-all duration-300 bg-[var(--bg-primary)]"
            >
        ...
        <div className="relative z-10 mt-12">
          <div className="flex justify-between items-end mb-4">
            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-primary)] opacity-40 font-jetbrains">Progress_Index</span>
...
const StatLine = ({ icon, label, value, sub, unit }: { icon: any, label: string, value: any, sub?: string, unit?: string }) => (
  <div className="flex items-center justify-between group/stat">
    <div className="flex items-center gap-4">
      <div className="opacity-50 text-[var(--text-primary)] group-hover/stat:opacity-100 group-hover/stat:text-[var(--accent-primary)] transition-all duration-300 scale-90 group-hover/stat:scale-100">{icon}</div>
      <span className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-40 group-hover/stat:opacity-60 transition-all font-jetbrains">{label}</span>
    </div>
    <div className="flex flex-col items-end">
      <div className="text-2xl font-black font-playfair italic leading-none text-[var(--text-primary)] group-hover/stat:text-[var(--accent-primary)] transition-colors">
        {value}
        {unit && <span className="ml-2 text-xs opacity-40 font-jetbrains not-italic font-bold">{unit}</span>}
      </div>
      {sub && <span className="text-[9px] font-jetbrains font-bold uppercase opacity-50 mt-1 tracking-tighter">{sub}</span>}
    </div>
  </div>
);
