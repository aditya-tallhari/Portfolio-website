...
import { ChevronLeft, Save, GraduationCap, Plus, X, Image as ImageIcon, Upload } from 'lucide-react';
import Link from 'next/link';

import { Toast, ToastContainer, createToastHelpers } from '../../components/Toast';
import { useAuth } from '../../context/AuthContext';
import { addEducation, Education } from '@/lib/api';
...
export default function AddEducationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message || 'Failed to add education record');
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20">
      ...
      <div className="mb-10">
        <p className="text-[10px] uppercase tracking-[0.4em] text-[var(--accent-primary)] font-jetbrains mb-1">New Entry</p>
        <h1 className="text-3xl font-playfair font-black text-[var(--text-primary)]">Add Academic Milestone</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="space-y-6 p-8 rounded-2xl bg-[var(--bg-secondary)] border border-[var(--border-primary)]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div className="space-y-2">
              <h3 className="text-xs font-bold text-[var(--text-primary)]">Image Guidelines</h3>
              <ul className="text-[10px] space-y-1 opacity-60 list-disc ml-4 font-jetbrains">
                <li>Best aspect ratio: 16:9 or 3:2</li>
                <li>Minimum width: 1200px for best quality</li>
        ...
        <button disabled={isSaving} type="submit" className="w-full py-5 rounded-2xl bg-[var(--accent-primary)] text-white font-black uppercase tracking-[0.2em] hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 disabled:scale-100 flex items-center justify-center gap-3 font-jetbrains text-sm">
          {isSaving ? 'Synchronizing...' : <><Save size={18} /> Save Education Record</>}
        </button>
      </form>
