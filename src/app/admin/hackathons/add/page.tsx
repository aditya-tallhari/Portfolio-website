...
export default function AddHackathonPage() {
  ...
  const validate = () => {
    ...
    if (!form.achievement.trim()) e.achievement = 'Achievement is required (e.g. "1st Place - $5000 Prize")';
    if (!form.date.trim()) e.date = 'Date is required (e.g. "March 2024")';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createHackathon(fd, token);
    ...
    <>
      ...
      <div className="flex items-center gap-4 mb-10">
        ...
        <div>
          ...
          <h1 className="text-3xl font-playfair font-black text-[var(--text-primary)]">Add New Hackathon</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
          <Field label="Description *" error={errors.description}>
            <textarea rows={4} placeholder="What was the hackathon about? What problem did you solve?" value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} className={`${inputClass} resize-none`} />
          </Field>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}>
          <Field label="Hackathon Image (optional)">
            <div
              className={`relative group h-40 rounded-2xl border-2 border-dashed transition-all cursor-pointer overflow-hidden flex flex-col items-center justify-center gap-3 ${
                previewUrl ? 'border-[var(--accent-primary)]/40' : 'border-[var(--border-primary)] hover:border-[var(--accent-primary)]/40 bg-[var(--text-primary)]/[0.02] hover:bg-[var(--text-primary)]/[0.04]'
              }`}
            ...
            >
              {previewUrl ? (
                ...
                <>
                  ...
                  <p className="text-[9px] text-[var(--text-primary)] opacity-20 font-jetbrains">PNG, JPG or WebP (max 5MB)</p>
                </>
              )}
              <input
                id="image-upload"
